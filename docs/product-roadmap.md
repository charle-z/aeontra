...
| 10 Codex harness compatibility spike | Signed source candidate implemented; release acceptance pending | Stock Codex 0.147.0 is pinned by archive and binary SHA-256. The source unit launches `codex exec` in the trusted workcell through the strict credential-free loopback Responses adapter and keeps multiagent disabled. Exact-head CI, signed release, restart/resume and real GPT Web acceptance remain separate gates. |

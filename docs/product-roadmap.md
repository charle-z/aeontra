...
| 5 complete persistent GitHub broker | In progress | The current candidate installs official `gh`, preserves normal full-account `gh auth login`, imports that authority into private Edge state, and exposes a first closed repository/PR/Actions permission preflight. Consequential PR, workflow and release operations remain separate slices. |

...
| 3A background start/status/stop | Merged and deployed; device acceptance pending | Backend and stable Front Door include direct durable background start/status/stop. The next signed Edge release/version still requires explicit release authorization before real-device acceptance. |
| 3B recovery/list/signal/cleanup | Merged and deployed; device acceptance pending | Closed signal/list/cleanup, restart reconciliation and live-preserving explicit cleanup are deployed; signed-release and real restart acceptance remain. |
...
| 4 persistent universal toolbox | Source and backend deployed; device acceptance pending | PRs #128/#129/#130 deployed lifecycle, services, resource/storage controls and access to the validated rootless engine for installed Podman/Docker/Compose clients. BuildKit/Compose and real restart acceptance await the next explicitly named signed release. |
| 5 complete persistent GitHub broker | Read-only slice deployed; device acceptance pending | PR #131 deploys official `gh`, preserves normal full-account `gh auth login`, imports that authority into private Edge state, and exposes the closed `project_github_status` repository/PR/Actions permission preflight. Consequential PR, workflow and release operations remain separate slices; real-device acceptance awaits an explicitly named signed release after `p15.0.12`. |

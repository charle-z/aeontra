...
| 5 complete persistent GitHub broker | Read-only slice deployed; login/import verified; v3 device acceptance active | PR #131 deploys the closed `project_github_status` preflight. Real login/import succeeded on `p15.0.13`; PR #134 and `p15.0.14` delivered the required v2 bridge. Real installation exposed one enabled legacy Edge unit retaining the old process lock. The v3 candidate both signs pinned official `gh` and retires only fixed legacy unit names before consequential PR/workflow/release slices. |

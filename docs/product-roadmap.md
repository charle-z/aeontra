...
| 5 complete persistent GitHub broker | Read-only slice deployed; login/import verified; atomic device handoff active | PR #131 deploys the closed `project_github_status` preflight. Real login/import succeeded on `p15.0.13`; PR #134 and `p15.0.14` delivered the v2 bridge. PR #135 and signed `p15.0.15` bind pinned official `gh`, but real activation rolled back because `disable --now` stopped the legacy caller before the managed unit could take ownership. The corrective disables legacy persistence first and uses fixed systemd conflicts for the atomic process replacement. |

...
| 3B recovery/list/signal/cleanup | Final sandbox-leader correction in progress | `p15.0.21` recovered the same live process after restart with continuous cursor output, proving journal/worker reconciliation. Its closed interrupt still targeted Bubblewrap's outer supervisor instead of the `--new-session` inner leader, so the workload required scoped operator TERM and H3B remains unaccepted. The candidate consumes Bubblewrap's server-owned `--info-fd`, revalidates the reported inner PID/PGID/start ticks/owner and binds the sandbox to its durable worker. |

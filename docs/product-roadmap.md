...
| 4 persistent universal toolbox | Core/services deployed; resource-control candidate pending | PRs #128/#129 deployed create/status/arbitrary exec/install/repair, named service lifecycle and explicit cleanup. The current candidate adds verified configurable CPU/memory/process limits. Direct rootless container/Compose/BuildKit operations, storage acceptance, signed release and real restart proof remain. |

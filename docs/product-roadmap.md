...
| 3B recovery/list/signal/cleanup | Final corrective candidate after orphan rejection | `p15.0.20` preserved output across restart, but a stale journal identity was marked terminal while its private worker and Bubblewrap child remained alive; cleanup then removed their metadata. The candidate recovers a live owner-only worker identity and refuses terminal cleanup while either exact private identity is alive. |
...
| 4 persistent universal toolbox | Post-restart validation in progress | `p15.0.20` recovered the existing toolbox, persisted its rootfs, installed tools, reached the real rootless Podman socket and kept a durable service. The container survived the coordinated Edge restart; final public service/marker verification and cleanup remain. |

...
| 3A background start/status/stop | Merged and deployed; device acceptance pending | Backend and stable Front Door include direct durable background start/status/stop. `p15.0.13` is installed on the real Edge; foreground/background acceptance remains after the signed CLI lifecycle correction. |
| 3B recovery/list/signal/cleanup | Merged and deployed; device acceptance pending | Closed signal/list/cleanup, restart reconciliation and live-preserving explicit cleanup are deployed; real restart acceptance remains after the signed CLI lifecycle correction. |
| Safe Edge checkout synchronization | Merged and deployed; device acceptance pending | Closed brokered status/fetch/preview/fast-forward and exact durable plans are deployed; signed-release and real Edge acceptance remain. |
| 4 persistent universal toolbox | Source and backend deployed; device acceptance pending | PRs #128/#129/#130 deployed lifecycle, services, resource/storage controls and access to the validated rootless engine for installed Podman/Docker/Compose clients. `p15.0.13` is installed; BuildKit/Compose and real restart acceptance remain. |
| 5 complete persistent GitHub broker | Read-only slice deployed; login/import verified; lifecycle package correction active | PR #131 deploys the closed `project_github_status` preflight. Real login/import succeeded on `p15.0.13`; PR #134 and `p15.0.14` delivered the v2 bridge. The operator completed the exceptional handoff from an unpackaged legacy service. PR #137 and signed `p15.0.17` removed the obsolete unit conflicts, then real activation exposed the root updater sandbox's missing fixed `/usr/local/bin` write path before unit installation. The corrected signed package and real-device acceptance are the next gate. |

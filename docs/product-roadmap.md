...
| 7 worktrees and deterministic parallelism | Implemented in source; acceptance pending | Server-owned exact-base worktrees, one-writer leases, monotonically fenced recovery, registered workspaces, explicit clean-only cleanup and retained worker branches are connected to the Edge. Exact-head CI, signed release and real-device restart acceptance remain. |
...
| 9 durable task graph and multiagent | Implemented in source; acceptance pending | `project_task_*` binds one to four durable worker jobs to distinct fenced worktrees, workspaces and independent stock Codex runtimes, reconciles them after server restart, cancels idempotently and preserves branches/evidence. Exact-head CI, signed release and real parallel GPT Web acceptance remain. |

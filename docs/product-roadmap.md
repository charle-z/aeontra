...
| P17 durable objective supervisor | Partially implemented | The managed model-turn completion gate now enforces explicit `active`, `blocked` and `complete` states and rejects declared pending work. Semantic objective evaluation, fair pending-turn attention, reviewed integration, compact handoff and content-free efficiency metrics remain planned. |

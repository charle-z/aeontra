...
| 3B recovery/list/signal/cleanup | Final sandbox identity-readiness correction in progress | `p15.0.21` proved restart reconciliation but targeted Bubblewrap's outer supervisor. `p15.0.22` switched to the server-owned `--info-fd`; its first real start exposed that Bubblewrap reports the inner child just before `--new-session` establishes `PGID == PID`, so readiness failed closed although the bounded probe ran to completion. The candidate waits a fixed two seconds for that exact transition while continuously rejecting disappearance, PID reuse and foreign ownership. H3B remains unaccepted until the next signed real restart/signal gate. |

...
| `MCP_DEVBOX_SANDBOX_RUNNER_WORKSPACE_ROOT` | private runner mapping | Runner only; sensitive host path | existing absolute directory, disjoint from state; either one direct repository or a multi-repository root whose direct child scopes are selected server-side | `/srv/aeontra/workspace`; only the selected repository is mounted at `/workspace` | Missing, symlink-invalid, inaccessible or overlapping root fails startup/execution. Ambiguous or unsafe scope selection fails closed. |

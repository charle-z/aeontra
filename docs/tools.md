...
| `project_toolbox_create` | 0/1/1/1 | Create or recover one persistent Debian rootfs through the registered workspace's user-owned rootless engine. Optional CPU, memory and process limits are range-checked; omitted values receive broad server-owned defaults. The base, private container identity, packages and caches persist until explicit cleanup. |
| `project_toolbox_status` | 1/0/1/1 | Return only the opaque toolbox id, lifecycle state, verified base image identity, applied CPU/memory/process limits and timestamps; host paths, engine socket and container identity remain private. |

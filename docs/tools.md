...
| `project_browser_create` | 0/0/1/1 | Create or reuse one durable Edge-private browser session with either public HTTP(S) egress or one exact high-port loopback origin. Profile paths, cookies, proxy and browser process identity remain private. |
| `project_browser_status` | 1/0/1/0 | Read one bounded browser-session summary with opaque identity, safe URL without query/fragment, title, state, revision and timestamps. |
| `project_browser_list` | 1/0/1/0 | List at most 20 safe browser-session summaries for one registered project and Edge target. |
| `project_browser_run` | 0/1/1/1 | Execute up to 32 closed navigation/locator actions in one ephemeral Chromium instance inside a narrow Bubblewrap boundary. It accepts no JavaScript, headers, cookies, executable, flags, paths, proxy or CDP endpoint; external page interaction may submit effects. |
| `project_browser_artifact_read` | 1/0/1/0 | Read one bounded base64 chunk from an Edge-private JPEG artifact by opaque session/artifact identities and byte offset. |
| `project_browser_close` | 0/1/1/0 | Idempotently close one non-busy session while preserving its private profile and artifacts until explicit cleanup. |
| `project_browser_cleanup` | 0/1/1/0 | Explicitly remove only closed browser sessions, exact private profiles and exact JPEG artifacts bound to the selected project. Ready/busy sessions are preserved. |

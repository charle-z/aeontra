...
func (f *fakeValidationService) RunTestsIn(approve bool, cwd string, extra ...string) (string, error) {
...
func TestRegisterValidationDefinesStableContractsAndRoutesHandlers(t *testing.T) {
	...
	wantDescriptions := map[string]string{
		"run_tests":                  "Run the project's configured test command (allowlisted). Optional cwd is jailed under the workspace. In ask mode, set approve=true to run.",
	...
	calls := []struct {
		...
		{name: "run_tests", args: `{"approve":true,"extra":["-run","TestOne"],"cwd":"/repos/project"}`, want: "tests-result"},
	...
	wantCalls := []string{
		"tests:/repos/project", "-run", "TestOne", "approved",

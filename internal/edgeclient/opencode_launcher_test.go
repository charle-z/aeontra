...
func TestBoundedSinkFailureSignalDoesNotExposeRawOutput(t *testing.T) {
	cases := []struct {
		...
		{`{"type":"error","error":{"name":"UnknownError","data":{"message":"spawn rg ENOENT"}}}`, "not_found_ripgrep"},
		{`{"type":"error","error":{"name":"UnknownError","data":{"message":"ENOENT: no such file or directory, open '/workspace/calc/calc.go'"}}}`, "not_found_workspace"},
		{`{"type":"error","error":{"name":"UnknownError","data":{"message":"spawn /bin/sh ENOENT"}}}`, "not_found_shell"},

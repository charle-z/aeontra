...
public catalog remains 67 tools with hash
...
Measure the standard long-running agent workflows and implement only the smallest
compact orchestration primitive justified by the result. Reuse existing policy,
audit, repository and validation paths. Edge work may then begin with device identity,
one-use pairing and leased-task replay tests. Do not expose a remote shell, pair a real
device, install WSL automatically, or begin Parrot/HTB authority.

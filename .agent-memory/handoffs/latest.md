...
to one, response bytes from 2050 to 406, and repeated bytes from 259 to zero.

## Next safe step

Close the Step 2 gates and commit, then implement persistent bounded telemetry/logs.
After that, add the bounded result store before Edge device identity, one-use pairing
